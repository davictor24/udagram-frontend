...
export class ApiService {
  ...
  get(endpoint): Promise<any> {
    const url = this.getMicroserviceEndpoint(endpoint);
  ...
  getMicroserviceEndpoint(endpoint): string {
    const match = /^(\/[^\/]+)(.*)/g.exec(endpoint);
    const microservice = match[1];
    const path = match[2];
    if (microservice == '/feed') return `${API_HOST_FEED}${microservice}${path}`;
    else if (microservice == '/users') return `${API_HOST_USERS}${microservice}${path}`;
